...
import Image1 from "../../../assets/av2.png";
...
const Profile = (props: Props) => {
  ...
  const [isPasswordChangeModalOpen, setPasswordChangeModalOpen] =
    useState(null);
  const [isEmailChangeModalOpen, setEmailChangeModalOpen] =
    useState<null>(null);
  ...
  return (
    <>
      {isLoading ? (
        ...
        <section className="bg-[#F2F7FF] rounded-md shadow-sm max-w-6xl mx-auto mt-20">
          <div className="bg-gradient-to-tr from-blue-500 to-violet-500 h-44 w-full rounded-t-md">
            <button
              onClick={() => setEditData(currentUser)}
              className="bg-white hover:bg-slate-300 transition-all duration-200 m-5 py-1 float-right px-5 font-medium rounded-sm "
            >
              Edit
            </button>
          </div>
          ...
          <div className="p-5 relative">
            <div className="avatar absolute -top-20">
              <div className="w-28 rounded-full ring ring-[#F2F7FF] ring-offset-base-100 ring-offset-2">
            ...
            <div className="mt-8">
              <div className="">
                <p className="capitalize text-xl font-montserrat font-bold">
                  {currentUser?.name}
                </p>

                <p className="flex items-center gap-2 mt-2 font-medium text-slate-500">
                  <AiOutlineMail /> {currentUser?.email}
                </p>
                <p className="flex items-center gap-2 mt-1 font-medium text-slate-500">
                  <IoCallOutline /> {currentUser?.phone}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-5">
            <h3 className="text-xl font-medium border-b pb-2 border-gray-300">
              Others Information
            </h3>
            {currentUser?.role === "student" && (
              <div className="mt-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-5">
                  <Link
                    target="blank"
                    to={`https://leetcode.com/${currentUser?.leetCode}`}
                    className="inline-block shadow-lg p-2 shadow-gray-300 rounded-full ring-1 ring-violet-300 cursor-pointer hover:scale-110 transition-all duration-200 text-gray-600 hover:text-violet-600"
                  >
                    <svg
                  ...
                  <Link
                    target="blank"
                    to={`https://codeforces.com/profile/${currentUser?.codeForces}`}
                    className="inline-block shadow-lg p-2 shadow-gray-300 rounded-full ring-1 ring-violet-300 cursor-pointer hover:scale-110 transition-all duration-200 text-gray-600 hover:text-violet-600"
                  >
                    <svg
                      className="h-6 fill-current"
                  ...
                  <Link
                    target="blank"
                    to={`https://www.codechef.com/users/${currentUser?.codeChef}`}
                    className="inline-block shadow-lg p-2 shadow-gray-300 rounded-full ring-1 ring-violet-300 cursor-pointer hover:scale-110 transition-all duration-200 text-gray-600 hover:text-violet-600"
                  >
                    <svg
                      className="h-6 fill-current"
                  ...
                  <Link
                    target="blank"
                    to={`https://atcoder.jp/users/${currentUser?.atCoder}`}
                    className="inline-block shadow-lg p-2 shadow-gray-300 rounded-full ring-1 ring-violet-300 cursor-pointer hover:scale-110 transition-all duration-200 text-gray-600 hover:text-violet-600"
                  >
                    <svg
                      className="h-6 fill-current"
            ...
            <button
              onClick={() => setEmailChangeModalOpen(currentUser?.email)}
              className="mt-5 cursor-pointer flex items-center gap-2 bg-slate-300 px-2 rounded-sm py-0.5 text-sm"
            >
              <FiMail size={16} /> Change Email
            </button>
            <button
              onClick={() => setPasswordChangeModalOpen(currentUser?._id)}
              className="mt-5 cursor-pointer flex items-center gap-1 bg-slate-300 px-2 rounded-sm py-0.5 text-sm"
            >
              <CiLock size={20} /> Change Password
            </button>
          </div>
        </section>
      )}
      ...
      {editData && (
        <ProfileEditModal editData={editData} setEditData={setEditData} />
      )}
    </>
  );
};
